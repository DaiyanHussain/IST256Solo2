...
function createBlogPostCard(blogPost){
    ...
    contentElement.testContent = blogPost.content;
...
function createLikeButton(likes) {
    ...
    heartIcon.src = 'resources/heart logo.webp';
    heartIcon.alt = 'Like';
...
function createCommentsElement(blogPost) {
    ...
    blogPost.comments.forEach((comment, index) => {
        ...
        userIcon.classList.add('heart logo.webp');
        userIcon.src = 'resources/user logo.jpeg';
        userIcon.alt = 'user';
...
function createCommentForm (blogPostId) {
    ...
    commentForm.addEventListener('submit', async (event) => {
        ...
        try {
            const response = await fetch(`/blog/${blogPostId}/comment`, {
                method: 'POST',
                headers: {
...
document.getElementById('blogPostForm').addEventListener('submit', async (event) => {
    ...
    try {
        ...
        if (!response.ok) {
            throw new Error('Failed to create blogPost. Please try again.');
        }
        event.target.reset();
